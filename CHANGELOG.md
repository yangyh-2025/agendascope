...
### 模型池核心（新增 `app/llm/model_pool.py`）

- **8 模型池**：SiliconFlow 4（GLM-Z1-9B/GLM-4-9B/Qwen3.5-4B/DeepSeek-R1-Qwen3-8B，并发8/QPS10）+ 智谱 3（GLM-4.7-Flash 并发1、GLM-4-Flash-250414 并发2、GLM-4V-Flash 并发10）+ 讯飞星辰 1（QPS2/并发2）
- **per-model 独立限流**：`DistributedSemaphore`（跨进程 Redis 租约，解决 detection/naming 多 worker 争抢）+ QPS 令牌桶，不同模型互不影响
- **综合并发**：全局并发闸门 = 各模型并发之和（abba 安全顺序防死锁），总吞吐 = 8+1+2+10+2 = 23 路
- **调度**：每请求选最空闲模型（in-flight 最少 → QPS 余量最大）
- **失败转移 + 熔断**：单模型失败自动换其他模型；连续 3 次失败熔断冷却 60s；全部不可用抛错触发既有降级链
- **配置**：`LLM_POOL`（.env JSON 数组），未配置退化为单模型（兼容 LLM_API_*）；compose 透传 `LLM_POOL`

### 附带

- **启用 snapshot-worker**（`compose.deploy.yml`）：热点 TOP10/地图下钻的显著性数据源此前被精简部署禁用，导致显著性全 0、热点排序失效；现启用（30min 间隔低内存适配），每轮 50 议题快照 ~20s 完成
- **测试隔离**：`tests/conftest.py` 清 LLM env（防开发机 .env 污染单测）；新增模型池单测 5 个（调度/失败转移/全失败/legacy）
